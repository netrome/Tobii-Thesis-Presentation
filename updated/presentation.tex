...
\plainslide{\acrfull{gans}}{
    ...
    \begin{itemize}
        ...
        \item These can be convolutional neural networks
        \item The discriminator is trained to distinguish fake and real images
...
\plainslide{\acrshort{gans} in practice}{
    \begin{itemize}
        ...
        \item Mode-collapse
        \item Lots of variants proposed (\acrshort{wgan}, Progressive \acrshort{gan}, AC\_GAN, LSGAN, BEGAN, DRAGAN, DCGAN etc...)
...
\plainslide{Measuring the usability of the data}{
    Historically difficult to evaluate \acrshort{gans}

    Train classifier separately on both generated and real data.
}

\plainslide{Generating labeled data}{
    No special cases needed - there is no difference between labeled and unlabeled data.
}

\sectionslide{Experiments and Results}

\plainslide{Tested models}{
    WGAN, VAE, AEGAN (Progressive GAN).

    Tested on synthetic data as well as G6 data.

    Additional toy experiments on the MNIST and iris data sets.
}

\plainslide{Network architectures}{
    TODOslide
}
...
\sectionslide{Concluding remarks}

\statementslide{Questions?}{images/closed.png}

\statementslide{Thank you}{images/opened.png}
